...
let currentIngredients = []; // Store ingredients for current meal entry
...
function setupEventListeners() {
    ...
    document.getElementById('newIngredientName').addEventListener('keypress', function(e) {
        if (e.key === 'Enter') {
            e.preventDefault();
            addIngredient();
        }
    });
    document.getElementById('newIngredientAmount').addEventListener('keypress', function(e) {
        if (e.key === 'Enter') {
            e.preventDefault();
            addIngredient();
        }
...
function addIngredient() {
    ...
    if (!name || !amount) {
        showToast('Please enter both ingredient name and amount', 'error');
        return;
    }
...
function displayIngredients() {
    ...
    container.innerHTML = currentIngredients.map((ingredient, index) => `
        <div class="ingredient-item">
            ...
            <button type="button" class="ingredient-remove" onclick="removeIngredient(${index})" title="Remove ingredient">
                âœ•
            </button>
        </div>
    `).join('');
...
function saveMealEntry(e) {
    ...
    const mealData = {
        ...
        ingredients: currentIngredients.length > 0 ? JSON.stringify(currentIngredients) : '',
...
function createMealCard(meal) {
    ...
    const walkInfo = meal.walkDistance > 0 ? 
        `ðŸš¶ ${meal.walkDistance} mi, ${meal.walkDuration} min (${meal.walkSpeed} mph)` : '';
    const glucoseInfo = meal.postMealGlucose ? 
        `ðŸ“Š ${meal.postMealGlucose} mg/dL` : '';
    ...
    if (meal.ingredients) {
        try {
            ...
            if (ingredients.length > 0) {
                ingredientsDisplay = `<div class="entry-ingredients">
                    <strong>ðŸ“ Ingredients:</strong>
                    <ul class="ingredient-list-display">
                        ${ingredients.map(ing => `<li>${ing.name} - ${ing.amount}</li>`).join('')}
                    </ul>
                </div>`;
            }
        } catch (e) {
            ...
            if (meal.ingredients) {
                ingredientsDisplay = `<div class="entry-detail">ðŸ“ ${meal.ingredients}</div>`;
            }
        }
    }
    ...
    return `
        <div class="entry-item">
            ...
            ${meal.notes ? `<div class="entry-detail">ðŸ’­ ${meal.notes}</div>` : ''}
    ...
    if (recipes.length === 0) {
        ...
        listElement.innerHTML = recipes.map(recipe => `
            <div class="recipe-item">
                ...
                <button class="delete-btn" onclick="deleteRecipe('${type}', '${recipe.id}')">ðŸ—‘ï¸</button>
            </div>
        `).join('');
    }
}
    ...
    if (items.length === 0) {
        ...
        listElement.innerHTML = items.map(item => `
            <div class="shopping-item ${item.checked ? 'checked' : ''}">
                ...
                <button class="delete-btn" onclick="deleteShoppingItem('${item.id}')">ðŸ—‘ï¸</button>
            </div>
        `).join('');
    }
}
